...
@interface RHAddImageView () <UIImagePickerControllerDelegate, UINavigationControllerDelegate>
{
...
- (instancetype)initWithFrame:(CGRect)frame
    ...
    if (self = [super initWithFrame:frame]) {
        _deleteImage = [UIImage imageNamed:@"RHdeletepicyure"];
        _addImage = [UIImage imageNamed:@"RHaddpicture"];
        ...
        tempFrame.size.height = _imageHeight;
...
- (void)deleteImage:(UIButton *)btn {
    ...
    for (int index = 0; index < _tagArray.count; index++) {
        ...
        if (_currentSection - 1 == btnSection) {
            if ([_delegate respondsToSelector:@selector(RHAddImageViewFrameDidDecrease:)]) {
                [_delegate RHAddImageViewFrameDidDecrease:self];
            }
        }
...
- (void)p_addImgView {
    ...
    self.frame = frame;
    if (_currentSection + 1 == btnSection) {
        if ([_delegate respondsToSelector:@selector(RHAddImageViewFrameDidIncrease:)]) {
            [_delegate RHAddImageViewFrameDidIncrease:self];
        }
    }
    _currentSection = btnSection;
}
